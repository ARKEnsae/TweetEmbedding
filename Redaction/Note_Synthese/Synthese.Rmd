...
Les modèles *word2vec*, développés par une équipe de recherche chez Google, sont parmi les plus célèbres.
...
La prédiction en elle-même ne nous intéresse pas, c’est plutôt le poids du classifieur en sortie du modèle qui correspondra aux *word-embeddings* de dimension $dim$. 

*Skip-gram* est un réseau de neurones à deux couches avec deux matrices initialisées en générant des lois normale $\mathcal N(0,1)$ :
...
Pour cela, on tire au hasard^[Pour éviter que les mots trop fréquents, souvent peu informatifs, soient sur-entraînés, on effectue un tout d'abord un sous-échantillonnage (*subsampling*).] un mot focus pour lequel on tire un mot *contexte* au hasard dans une fenêtre $w$. 
...
Ces données correspondent à un ensemble de tweets postés en France entre 2011 et 2018, supposés être représentatifs de l'ensemble de tweets nationaux publiés durant cette période. Nous avons mobilisé un premier échantillon de 1,3 million de tweets pour entraîner notre modèle \emph{word2vec} et un second échantillon composé de 4 200 tweets mensuels sur la période pour construire notre indice mensuel de sentiment. 

\textbf{Tweets annotés sur les transports urbains}

Cette base est composée d'un ensemble d'environ 23 000 tweets qui concernent les transports urbains (trains SNCF, métros et bus de la RATP) annotés en termes de sentiment (+1 s'ils sont positifs, -1 s'ils sont négatifs).
...
Selon nos critères d'évaluation, les résultats obtenus en sortie du modèle *word2vec* nous confirment la qualité de l'entraînement du corpus de 1,3 million de tweets^[Avant de nous attaquer au jeu de données complet, nous avons également évalué notre modèle sur un corpus fictif afin de nous assurer de sa robustesse et de sa validité.] : 
...
Les tendances des deux indicateurs de sentiment diffèrent de celle de l'indicateur Camme. On peut toutefois remarquer que l'indice réalisé à partir du modèle 2 apporte une information pour prévoir^[Il le cause au sens de Granger (p-valeur de 0,02).] l'évolution de l'indicateur Camme. 
