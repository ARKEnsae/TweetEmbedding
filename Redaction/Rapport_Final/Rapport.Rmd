...
Ces deux matrices sont initialisées en générant des lois normale $\mathcal N(0,1)$. Elles sont ensuite mises à jour, grâce aux couples `[focus, contexte]` construits à partir du contexte (partie \@ref(subsec:baseentrainement)), par un algorithme de descente de gradient.
...
Afin de créer un indice mensuel de sentiment moyens des tweets, nous allons utiliser les vecteurs-mots en sortie du modèle *word2vec* couplé avec une base de tweets de la SNCF qualifiés de positifs ou négatifs.
...
Nous comparerons deux approches : une première basée sur les sentiments moyens de chaque mot des tweets (partie \@ref(sec:sentiments)) qui servira de référence (*baseline*) pour évaluer l'efficacité de la seconde approche basée sur les *word-embeddings* (partie \@ref(sec:wordembeddings)) .
...
Afin d'évaluer l'efficacité du modèle et de sélectionner le $\gamma$ qui présente les meilleures performances, nous séparons le corpus de la SNCF en une base d'entraînement (environ 16 000 tweets) et une base de test (environ 7 000 tweets). La base d'entraînement sert à calculer les sentiments moyens des mots^[13 \% des mots du vocabulaires et 1 \% des mots utilisés dans les tweets de la base test sont absents de la base d'entraînement. Pour ces mots, le sentiment moyen $\alpha_i$ est fixé à 0.], et la base de test permet d'estimer le modèle, c'est-à-dire prédire pour chaque tweet un sentiment, que l'on compare au vrai sentiment.

On choisit $\gamma$ tel que l'*accuracy* sur la base de test -- c'est-à-dire le taux de tweets dont le sentiment est bien prédit -- soit maximale (figure \ref{fig:max_baseline})^[
Dans notre base de test la proportion de tweets positifs (43 \%) est proche de la proportion de tweets négatifs (57 \%). 
...
]. L'*accuracy* maximale vaut 89,1 \% pour $\gamma^* = -0,14$. On constate une nette amélioration par rapport au cas où $\gamma = 0$ (70,5 \%), qui correspond au seuil « naturel » puisqu'il se situe au centre de l'intervalle des valeurs possibles pour $\alpha_i$. 
...
Cette valeur de l'*accuracy* correspond au seuil $\gamma = 0,5$. Comme dans la partie précédente, nous avons déterminé le seuil optimal et comme celui-ci était très proche de 0,5 ($\gamma^* = 0.495$), pour une *accuracy* de seulement 70,0 \%, nous avons conservé le seuil médian de $0,5$.
], valeur inférieure au modèle de référence de la partie \@ref(sec:sentiments) (89,1 \%). 


### Les limites des modèles utilisés 

La bonne performance du modèle de référence (sentiments moyens des mots par tweets) par rapport au modèle mobilisant les *word-embedding* peut s'expliquer par plusieurs facteurs. 
...
#### Le processus d'annotation utilisé par la SNCF
...
En effet, nous ne disposons pas de la méthodologie qui a permis à la SNCF d'annoter ses tweets et nous avons pu relever quelques incohérences dans les sentiments attribués à certains tweets. Cela peut laisser supposer que l'annotation n'a pas été purement manuelle.
...
#### Le domain-shift\footnote{Les éléments en lien avec le \emph{dataset shift} proviennent de \cite{Candela}}
...
L’idée n’est pas ici de modéliser mathématiquement \colorbox{BurntOrange}{NDBP à conserver ?}^[
...
La distribution $\mathbb P(y\mid x_0)$ (de  l’indice mensuel sachant le jeu de tweets idéal utilisé) est considérée comme étant la même pour les deux jeux de tweets utilisée (base « sncf » et base de tweets postés en France entre 2013 et 2017). 
En revanche, cette distribution est modifiée si $F$ est modifiée.
] le *domain shift* mais de remarquer que la base de tweets de la SNCF semble en effet qualitativement être une base particulière : de part le vocabulaire spécifique qui y est employé^[
...
Nous testé les deux modèles sur une nouvelle base de test de tweets annotés^[
...
52,0 \% des mots du vocabulaire et 12,7 \% des mots de la base test de github ne sont pas dans la base d’entraînement SNCF et 52,8  \% des mots du vocabulaire et 13,0 \% des mots ne sont ni dans la base d’entraînement SNCF ni dans le vocabulaire des \emph{word-embeddings}. 
Ainsi, les mots de la base test « github » qui ne sont pas dans la base d’entraînement « sncf » sont rarement dans les vecteurs-mots de \emph{word2vec}.
...
*A contrario*, l'indicateur synthétique issu de l'enquête Camme peut-être considéré comme CVS-CJO puisqu'il est construit à partir de soldes d'opinion CVS-CJO. Pour comparer notre indicateur à l'indicateur issu de Camme, deux solutions sont possibles :
  ...
  Pour cela, nous avons construit un nouvel indicateur synthétique en appliquant les poids associés à chaque variable aux soldes d'opinion bruts^[
...
Cela peut venir du fait que, dans les tweets utilisés pour construire les indicateurs mensuels, il y a bien plus de mots inconnus au modèle de référence basé sur les tweets SNCF (20,7 \%) qu'à notre modèle *word2vec* qui comporte de nombreux *vecteurs-mots* (13,6 \%).
...
L'indicateur de sentiment issu du modèle *word2vec* cause au sens de Granger l'indicateur synthétique issus de l'enquête Camme (p-valeur de 0,02).
...
En somme, l'exploitation des résultats de *word2vec* dans la construction d'un indice de sentiment de tweets permet, contrairement au modèle de référence, de construire un indicateur anticipé d'un mois permettant de prévoir l'indicateur synthétique de l'enquête Camme. 
Cela est vrai alors même que les deux indicateurs évoluent différemment en raison de leur philosophie différente : alors que l'indicateur Camme résume l'opinion des ménages sur des sujets très spécifiques (évolution du niveau de vie, du chômage, etc.), notre indicateur de sentiment mesure juste la positivité / négativité des mots employés dans des tweets, sans cibler de thématique particulière.

La construction d'un indice de sentiment basé sur les tweets pourrait évidemment être prolongée et améliorée. 
Un prolongement possible serait le prétraitement plus approfondi de la base de tweets. Les mots mal orthographiés (lettres en trop, oubli d'espace, mauvaise ponctuation\dots) accroissent en effet considérablement la taille du vocabulaire et la rareté des mots. 
Toutefois, cette correction doit être effectuée avec précaution ; les différentes orthographes du même mot peuvent parfois refléter des sentiments différents. 
Par exemple, « lol », « LOL » et « loool » sont peut-être utilisées dans des contextes différents : ironie, blagues, rires\dots

L'utilisation de modèles d'analyse de sentiment plus complexes, comme les réseaux de neurones récurrents, amélioreraient peut-être également la qualité de la prévision du sentiment d'un tweet, tout comme l'utilisation de bases d'entraînement contenant une plus grande gradation des sentiments des tweets (permettant notamment qu'ils soient catégorisés de « neutres »).
Nous pourrions également essayer de gérer avec plus de précision les mots manquants, en étudiant leur nature et l'impact de notre méthode d'imputation sur les prévisions de sentiments.


<!-- J'avais à un moment commencé ce paragraphe mais ça me parait trop bancale -->
<!-- Il serait fastidieux d'essayer d'interpréter les différences entre nos indicateurs et l'indicateur Camme.  -->
<!-- D'une part parce qu'il serait trompeur d'associer à une évolution de nos indicateurs l'occurence d'un événement politique ou d'un fait divers qui ne serait pas pris en compte dans les soldes d'opinion de l'enquête Camme^[ -->
<!-- Par exemple il serait fallacieux de dire que l'affaire Penelope Fillon a contribué à la baisse du sentiment entre décembre 2016 et janvier 2017 -->
<!-- ] -->

<!-- Nous reviendrons sur le modèle vectoriel à correction d'erreur (VECM) qui cherche à modéliser une relation avec plusieurs variables explicatives, plus complexe, dans un prochain éclairage. -->


<!-- - Sur les séries différenciées, notre indicateur cause (instantanément ? je crois pas, à revérifier) l’indicateur Camme (non retardé) au sens de Granger, il pourrait éventuellement aider à le prévoir.  Notre indicateur nous donne un indicateur avancé en termes de temps de l’enquête Camme, ce qui n’est pas le cas de la base git sncf.  -->
<!-- - Indiquer les idées que tu as eu concernant le PIB et pourquoi cela n’a pas abouti.  -->
<!-- - Conclusion de la partie : pas de proximité apparente et difficile d’en dire plus mais résultat surprenant en termes de prévision.  -->
<!-- * ouverture (éventuelle) sur le lien avec les événements d’actualité : dire ce qu’on a tenté et conclure qu’il est très difficile d’analyser les différences entre nos indicateurs et l’indicateur Camme.  -->
<!-- * ouverture (encore plus éventuelle) : sur si on observe un message intéressant sur une des 8 composantes de Camme.  -->
...
Au-delà de la positivité ou négativité d’un tweet, il demeure important de déceler les sujets sur lesquels les tweets portent pour créer un indice plus ciblé (via, par exemple, l'utilisation de *hashtags*).
En effet, l'opinion des Français évolue différemment entre 2011 et 2018 en fonction des sujets de société concernés (figure \@ref(fig:figconclu)).
...
\begin{figure}[htp]
{\centering \includegraphics[width =\textwidth]{img/graph_conclu}}
\captionsetup{margin=0cm,format=hang,justification=justified}
...
Les méthodes d’apprentissage étant en perpétuelle évolution, le modèle *word2vec* mis en exergue en 2013, connaît déjà des prolongements. 
Par exemple, son extension *fasttext*^[
...
] (\cite{Bojanowski}), traite de la rareté des mots en représentant le mot non seulement comme lui-même mais grâce à des sacs de $n$-grammes constitutifs. 
Par exemple, si $n = 3$, « WHERE » est représenté non seulement par `<where>` mais également `<wh, whe, her, ere, re>`. *word2vec* connaît également des modèles « concurrents », le plus connu étant certainement *GloVe* pour « *Global Vectors* » (\cite{Pennington}). 
...
1. La série initiale est pré-ajustées de certains effets déterministes (effets jours ouvrables et points atypiques) grâce à un modèle de régression linéaire dont les résidus suivent un modèle ARIMA (partie \@ref(sec:cjo)).
...
On considère ainsi que l'effet d'un lundi lundi est le même que l'effet d'un mardi, etc. jusqu'au vendredi et que cet effet est différent le samedi et le dimanche. Le modèle \@ref(eq:eqcjotd) devient :
\begin{eqnarray}
y_t=\beta_0 LY_t + \tilde\beta_1 \left(\sum_{j=1}^{5}N_{jt} - (N_{6t} + N_{7t})\right) + \varepsilon'_t
(\#eq:eqcjowd)
\end{eqnarray}

Dans notre cas, puisqu'il n'y a pas d'effet longueur du mois nous omettant le régresseur $LY_t$ dans \@ref(eq:eqcjotd) et \@ref(eq:eqcjowd). On considère qu'il y a un effet jours ouvrables significatif lorsque $\exists i\in \{1,2,\dots,6\}\::\:\beta_i\ne0$ ou lorsque $\tilde\beta_1\ne0$.

<!-- Si on veut simplifier on peut supprimer jusqu'ici. -->

Pour nos indicateurs de sentiment (modèle logistique à partir de *word2vec* ou modèle de référence construit à partir du sentiment moyen des mots), nous ne détectons pas d'effet jours ouvrables. 
...
Cela peut venir de la méthode de collecte : tirer les tweets tous les 1, 5, 10, 15, 20, 25 et 28 de chaque mois revient à tirer tous les jours de la semaine sauf 1^[
Si le 1^er^ jour du mois est un lundi alors, parmi les 4 200 tweets, 1 200 sont tirés un lundi, 0 le mardi et 600 les autres jours de la semaine.
].
...
L'étude de ceux de l'indicateur calculé à partir du modèle *word2vec* suggère que les tweets sont globalement plus pessimistes en septembre et février, et plus optimistes en avril, octobre et décembre.
