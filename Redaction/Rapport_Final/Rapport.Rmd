...
Ce premier modèle lexical de prédiction du sentiment utilise l'information des tweets labelisés pour déterminer un sentiment moyen par mot.
...
Afin d'évaluer l'efficacité du modèle, nous séparons le corpus de la base de tweets sur les transports en une base d'entraînement (environ 16 000 tweets) et une base de test (environ 7 000 tweets)^[
...
Il n'y a donc pas de problème de déséquilibre de classe : maximiser l'*accuracy* permet bien de maximiser la performance de notre algorithme.
...
La bonne performance du modèle lexical (sentiment moyen des mots par tweets) par rapport au modèle logit (sur les *word-embedding*) peut s'expliquer par plusieurs facteurs. 
...
Pour le modèle basé sur les *word-embedding*, il faut mécaniquement ajouter aux mots inconnus ceux pour lesquels nous ne disposons pas de vecteurs-mots. 
...
Une dernière explication possible pourrait être dû à ce qu'on appelle le \emph{« domain shift »} ou changement de domaine. 
...
La correction gamma (représentation paramétrique non linéaire de l’intensité des pixels) est par exemple une manière de pouvoir traiter le *domain shift* lié à l’utilisation d’appareil photos différents.
].

L’idée n’est pas ici de modéliser mathématiquement le *domain shift*^[
En quelque mot, pour modéliser mathématiquement un *domain shift* on considérerait une variable latente « idéale » $x_0$ (une base de tweets de référence), jamais observée mais qui influerait sur $y$ (l’indice mensuel).
...
Avec cette nouvelle base, l'*accuracy* du modèle basé sur les *word-embedding* est cette fois-ci supérieure à celle de l'approche lexicale (61,9 \% contre 55,9 \%).  

Cette nouvelle base test permet certainement de corriger une partie des biais évoqués plus haut qui pouvaient expliquer la surperformance du modèle lexical par rapport au modèle utilisant les vecteurs mots : 
...
- Le modèle de *word-embeddings* semble mieux réagir à la présence de mots inconnus, ici plus nombreux que précédemment^[
...
<!-- - (Re)Dire rapidement comment on passe du modèle logit à la construction de l’indicateur mensuel (en gros on fait la moyenne des sentiments des tweets par mois).  Et parler du fait que l’on retarde Camme pour la cohérence temporelles entre les deux. -->
<!-- - Graphique : superposer les séries : baseline SNCF, logit, Camme sur un graphique renormalisé par moyenne et écart-type (0,1).  -->
<!-- [- tests de racine unitaire nous indique que nos séries doivent davantage être utilisées en différences pour être analysées ? Faire le graphique des séries différenciées ? Si sert plus tard dans l’argumentaire…] -->

### Comparaison entre les séries {#subsec:compseries}

Le graphique \@ref(fig:bslogcam) présente l'indicateur de sentiment construit à partir de notre modèle *word2vec*, celui construit à partir du modèle lexical et l'indicateur synthétique brut issu de l'enquête Camme (retardé).
...
Par ailleurs, l'indicateur construit à partir du modèle lexical parait plus bruité que l'indicateur de sentiment construit à partir de notre modèle *word2vec*.
Cela peut venir du fait que, dans les tweets utilisés pour construire les indicateurs mensuels, il y a bien plus de mots inconnus au modèle lexical basé sur les tweets sur les transports urbains (20,7 \%) qu'à notre modèle *word2vec* qui comporte de nombreux *vecteurs-mots* (13,6 \%).

\begin{figure}[htp]
{\centering \includegraphics[width =\textwidth]{img/rmd-graphSentiments-1}}
\captionsetup{margin=0cm,format=hang,justification=justified}
\caption{Indicateur synthétique brut de confiance des ménages de l'enquête Camme (retardé) et indicateurs mensuels de sentiment construits à partir de nos modèles logit et lexical.}\label{fig:bslogcam}
\footnotesize
...
Cette méthode conclut sur le fait que la similarité entre l'indicateur Camme et notre indicateur de sentiment issu du modèle lexical est légèrement plus grande que celle entre l'indicateur Camme et l'indicateur de sentiment calculé à partir de notre modèle *word2vec*. 



<!-- - Analyse des proximités entre séries.  -->
<!-- * Algorithme de distance « élaboré » (Kendall je crois) indique que les deux séries (baseline SNCF et notre indicateur word2vec) sont tout aussi loin de Camme -->
<!-- * les séries Camme et Reg_log ne sont pas cointégrées et ne semblent donc pas avoir d’informations communes (faire la même chose entre Camme et baseline SNCF ?) -->
<!-- - Analyse de la saisonnalité de la série logit : très peu de saisonnalité (dire que c’est la même chose pour Camme). Pas d’effet longueur de mois, type de jour et effet pâques (tester la même chose sur CAMME et baseline pour voir si on arrive aux  même conclusions ? ). Si vraiment on voulait voir un effet, on pourrait dire que les mois les plus optimistes sont avril et octobres et le plus pessimiste : septembre.  -->
...
L'indicateur de sentiment issu du modèle *word2vec* cause au sens de Granger l'indicateur synthétique issus de l'enquête Camme (p-valeur de 0,05).
...
Ce n'est pas le cas pour le premier indicateur calculé à partir du modèle lexical (p-valeur de 0,58), malgré la similitude entre ces deux modèles (partie \@ref(subsec:compseries)).
...
En revanche, les mots inconnus y sont plus fréquents et le modèle obtenu présente de moins bons résultats en termes d'*accuracy* à la fois sur une base test issue cette base mais également provenant de la base sur les transports urbains.
]. 

# Discussion et prolongements {.unnumbered} 

Ce travail révèle que l'utilisation d'une méthode de *word-embedding* entraînée sur des tweets permet, grâce à un modèle simple, de construire un indicateur de sentiment pouvant être comparé à des indices d'opinion produits grâce à des enquêtes de la statistique publique. 
...
L’entraînement et le test du modèle logit dont les prédicteurs correspondent aux dimensions des *word-embedding* nous a permis de remarquer qu’en plus de représenter la proximité entre mots, le modèle *word2vec* permet de capter dans une certaine mesure le sentiment de phrases. 
