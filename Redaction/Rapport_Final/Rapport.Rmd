...
Finalement, nous retenons le modèle estimé par une **régression logit** sur la base en **gardant les mots-vides** et en affectant aux mots inconnus le **vecteur des mots très peu fréquents**. L'*accuracy* de ce modèle est de 69,8 \%^[Cette valeur de l'*accuracy* correspond au seuil $\gamma = 0,5$. Comme dans la partie précédente, nous avons déterminé le seuil optimal et comme celui-ci était très proche de 0,5 ($\gamma^* = 0.495$), pour une *accuracy* de seulement 70,0 \%, nous avons conservé le seuil médian de $0,5$.], valeur inférieure au modèle de référence de la partie \@ref(sec:sentiments) (89,1 \%). 
