...
Comme décrit dans la partie \@ref(subsec:word2vec), l'approche *Skip-gram* peut être vue comme un réseau de neurones avec trois couches avec : \colorbox{BurntOrange}{deux ou trois ? si trois, corriger aussi plus haut}
...
est ensuite *tokénisée* par la chaîne de caractère correspondant à un espace `" "` : on considère qu'il y a autant de mots de que chaînes de caractères séparées par un espace^[Les mots composés sont donc considérés comme plusieurs mots distincts.]. 

\begin{quote}
\LARGE \textbf{``}\normalsize \emph{Que pensez-vous de CE projet ?(i.e. : qu'avez-vous retenu en 10min?)} \LARGE \textbf{''}\normalsize
\end{quote}

Cette phrase est décomposée en 14 mots `[que, pensez, vous, de, ce, projet, i, e, qu, avez, vous, retenu, en, 10min]`.
...
1. Pour chaque phrase on effectue un sous-échantillonnage (*sub-sampling*). Pour chaque mot $w_i$ on note $z(w_i)$ la proportion d'apparition de ce mot, c'est-à-dire le rapport entre le nombre de fois que ce mot apparaît et le nombre total de mots. La probabilité de garder un mot le mot $w_i$ est : 
$$
...
2. Pour chaque phrase, on tire au hasard (selon une loi uniforme) un mot focus pour lequel on tire un mot *contexte* au hasard dans la fenêtre $w$ en imposant que les deux mots choisis soient parmi les mots sous-échantillonnés \colorbox{BurntOrange}{sinon on retire ? éventuellement ndbp}.
...
Pour mieux comprendre, nous remplaçons les mots non échantillonnés par « nonsubsampled ». La phrase devient `[nonsubsampled, pensez, nonsubsampled, nonsubsampled, ce, projet, nonsubsampled, e, qu, avez, vous, retenu, en, 10min]`.
...
- Pour chaque couple `[focus, contexte]`, la complexité du calcul de $\log\mathbb P(w_{focus}\vert w_{context})$ est proportionnelle à la taille du vocabulaire ;  
...
Le *negative sampling* est basé sur le concept de *Noise Contrastive Estimation* -- estimation contrastée du bruit -- selon lequel un bon modèle logistique devrait être capable de différentier un faux signal d'un vrai signal \colorbox{BurntOrange}{à clarifier ?}.
...
L'idée du *negative sampling* est donc de transformer un problème de classification multi-classes en un problème de classification binaire : pour chaque couple `[focus, mot2]`, on cherche à déterminer si `mot2` est dans le contexte de `focus`. Si c'est le cas, alors la décision $D=1$ (mot2 est positif et est le `contexte`), sinon $D=0$ (mot négatif, appartient à $(w_{neg,\,i})_{i=1..K}$). 
...
\colorbox{BurntOrange}{harmoniser avec la loi de proba définie plus haut pour bien qu'on comprenne qu'on fait référence à la mm chose}
...
\colorbox{BurntOrange}{reformuler pour la V2 devant el profesor Kim}
