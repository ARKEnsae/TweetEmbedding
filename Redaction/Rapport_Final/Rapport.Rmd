...
Pour construire ce corpus, nous construit 10 groupes de mots composés d’un couple (du type `[grand, petit]`) et de dix mots différents que l'on considère comme des mots contextes (`[taille, xl...]`). Le corpus fictif est formé de 10 000 phrases de neuf mots. Chacune de ces phrases est constitué de la façon suivante :
...
Après avoir évalué notre modèle sur ce corpus fictif, ,ous avons mis en œuvre les différentes techniques d'évaluation^[À l'exception de la méthode par \og jugement humain \fg{} puisque le corpus est ici créé fictivement par ordinateur sans prêter attention au réel sens des mots.] 
