...
Afin de créer un indice mensuel de sentiment moyen des tweets, nous allons utiliser les vecteurs-mots en sortie du modèle *word2vec* couplés avec une base de tweets annotée en termes de sentiment. Cette base contient un ensemble de tweets sur les transports urbains tous qualifiés de positifs ou négatifs .
Nous comparerons ensuite cet indicateur avec indicateur synthétique de confiance des ménages (Camme, Insee). 
...
$$
\begin{cases}
S_{2,\gamma}(t) = 0 &  \text{ si} \quad \mathbb{P}(Y_i = 1 | X_{i}) < \gamma \\
S_{2,\gamma}(t) = 1 &  \text{ si} \quad \mathbb{P}(Y_i = 1 | X_{i}) \ge \gamma
\end{cases}
$$
...
#### Test d'une modification de la source de la base de test 
...
L’entraînement et le test du modèle logit dont les prédicteurs correspondent aux dimensions des *word-embedding* nous a permis d’observer qu’en plus de représenter la proximité entre mots, le modèle *word2vec* permet de capter dans une certaine mesure le sentiment de phrases. 
