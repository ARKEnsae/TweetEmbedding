...
2. Pour chaque phrase, on tire au hasard (selon une loi uniforme) un mot focus pour lequel on tire un mot *contexte* au hasard dans la fenêtre $w$, tout en imposant que les deux mots choisis soient parmi les mots sous-échantillonnés^[
Si ce n'est pas possible alors on ne retient aucun couple `[focus, contexte]` pour cette phrase. Sinon, on ne fait un tirage du mot focus parmi ceux tels qu'il y a au moins un mot contexte échantillonné dans la fenêtre $w$.
].
...
En effet, pour chaque couple `[focus, contexte]`, la complexité du calcul de $\log\mathbb P(w_{focus}\vert w_{context})$ est proportionnelle à la taille du vocabulaire.
...
Le *negative sampling* est basé sur le concept du *Noise Contrastive Estimation* -- estimation contrastée du bruit -- où on cherche, à partir d'un modèle logistique, à différencier un vrai signal (un vrai couple `[focus, contexte]`) d'un faux (un bruit, i.e. : de faux couples `[focus, contexte]` qui seraient générés aléatoirement).
...
L'approche *softmax* peut être vue comme un problème de classification multiclasses : étant donné un mot `contexte` on estime la probabilité que les autres mots soient voisins (chaque classe étant un mot du vocabulaire).
...
Les méthodes quelques méthodes que nous avons retenues pour évaluer la qualité des vecteurs-mots obtenus sont décrites plus précisément dans l'annexe \@ref(annexe:commentEvaluer). 
Nous les appliquerons dans les parties suivantes. 
...
présentées en partie \ref{sec:commentEvaluer} sur les *word-embeddings* obtenus grâce à ce corpus fictif. 
...
], l'opération $\overrightarrow{Paris} - \overrightarrow{France} + \overrightarrow{Italie}$ (figure \ref{fig:acp_rome}) n'identifie pas \og Rome \fg{}(\colorbox{BurntOrange}{similarité cosinus de XXXXXXX}) dans les mots les plus proches mais d'autres villes comme \og Lyon \fg{} (similarité cosinus de 0,62). 
...
- L'*espace des individus*, qui est de dimension $p$, à qui on associe une métrique $M$ utilisée pour le produit scalaire. Dans la suite nous utiliserons $M =I_p$ la matrice identité : la norme et le produit scalaire associés à $M$ sont égaux à la norme et le produit scalaire eucliden.

- L'*espace des variables*, qui est de dimension $n$, à qui on associe une métrique $N=diag(p_1,...,p_n)$ avec $\sum_{i=1}^np_i=1$. La matrice $N$ représente le poids donné à chaque individus, par simplification nous utiliserons ici que des poids uniformes : $N=\frac{1}{n}I_n$. Afin de donner le même poids à toutes les variables, chaque variable est centrée-réduite : cela revient à centrer-réduire les colonnes de notre matrice $X$. Nous notons $Z =\bar X= (z_{ij})$  la matrice des données centrées et réduites^[Nous travaillons ici dans le cadre d'une ACP normée où la matrice $X$ a été centrée puis réduite. La réduction de $X$ a modifié les distances initiales entre individus ($\langle z_i,z_{i'}\rangle_M \neq \langle x_i,x_{i'}\rangle_M$). Cela n'aurait pas été le cas si la matrice $X$ avait été uniquement centrée (ACP non normée).].
...
Les vecteurs directeurs des axes sont définis de manière à maximiser la dispersion du nuage (son inertie) des individus projetés et conserver ainsi au mieux les distances entre les individus. L'inertie se définit comme la dispersion du nuage de points, c'est une généralisation de la notion de variance au cadre multivarié (dispersion des données mesurées sur $p$ variables) :
\begin{align*}
...
\\&=\sum_{j = 1}^p Var(z^j)\text{, avec } z^j = 
  ...
  \end{pmatrix}
s\\ &= p \text{ car les variables sont réduites}
\end{align*}
...
\underbrace{\|Zv\|_N}_{=Var(Zv)} =\underset{\| v \|_M = 1}{\mathrm{argmax~}} ^t\!v\ R v 
$$ 
...
- $v_k$ est un vecteur propre associé à la k\ieme{} valeur propre $\lambda_k$ de $R$ (les valeurs propres étant rangées par ordre croissant)  ; 
