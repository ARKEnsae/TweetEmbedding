...
Le modèle *Word2Vec* est une des méthodes de machine learning utilisée pour faire du *word embedding* --- plongement lexical. 
Elle permet de représenter, de manière vectorielle, l'ensemble des mots d'un corpus.
Ainsi, les mots qui apparaissent dans des contextes similaires (par exemple « bonjour » et « salut ») seront représentés par des vecteurs relativement proches.
L'objectif de cette partie est de décrire le fonctionnement de cet algorithme.

Deux architectures du modèles *Word2Vec* existent (voir graphique \@ref(fig:cbowskipgram)) :
...
## Description de la méthode et tests [Partie rédigée par Roro]
