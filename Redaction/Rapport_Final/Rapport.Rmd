...
La « sémantique vectorielle » est née dans les années 1950^[L’ouvrage \cite{Jurafsky} permet de retracer avec une grande richesse l’évolution des méthodes de NLP.] . C’est une méthode algébrique de représentation d’un document visant à réaliser des tâches diverses (détecter le plagiat, filtrer des articles\dots). Il est alors nécessaire de capter de nombreux types de proximités entre mots : les synonymes (automobile / voiture), antonymes (froid / chaud), connotations positives *versus* négatives (heureux / triste), etc. 

Un modèle répondant à ces exigences ne peut exister. Pour y répondre au mieux, la sémantique vectorielle puise son inspiration des travaux linguistiques des années 1950 et en particulier de l’ « hypothèse de distribution » selon laquelle un mot se définit par son environnement, dit autrement : les mots qui se produisent dans un contexte identique tendent à avoir des significations similaires^[Comme l'a écrit le linguiste britannique John Rupert Firth en 1957, « Vous connaîtrez un mot par ses fréquentations ».]. 

Les premiers modèles sémantiques^[Comme le *term frequency-inverse document frequency* (TF-IDF)] représentaient les relations entre mots grâce à des très grandes matrices dont les dimensions correspondaient à la taille du vocabulaire (contenant donc beaucoup de 0 et dites *sparses*). Les méthodes de *word-embedding* qui sont ensuite apparues ont permis de représenter chaque mot d’un dictionnaire par un vecteur de nombres réels denses (peu de 0) de plus faible dimension (en général entre 50 et 1000).  Si la réduction de dimension rend les vecteurs-mots moins facilement interprétables, elle a pour grand avantage de faciliter et d’accélérer les tâches d’apprentissage impliquant ces mots. 
...
Comme décrit dans la partie \@ref(subsec:word2vec), l'approche *Skip-gram* peut être vue comme un réseau de neurone avec trois couches avec : 
...
Ainsi, la ligne $i$ de la matrice $W=\frac{W_e+W_s}{2}$ donnera la représentation du $i$^ème^ mot du vocabulaire.
...
Chaque phrase^[Dans notre cas une phrase correspond à un tweet, même si ce tweet peut-être écrit en plusieurs phrases.]
est ensuite *tokénisée* par la chaîne de caractère correspondant à un espace `" "` : on considère qu'il y a autant de mots de que chaînes de caractères séparées par un espace. Ainsi, les mots composés ne seront donc pas pris en compte.
Par exemple dans la phrase « Que pensez-vous de CE projet ?(i.e. : qu'avez-vous retenu en 10min?) » est décomposée en 14 mots `[que, pensez, vous, de, ce, projet, i, e, qu, avez, vous, retenu, en, 10min]`.

Comme décrit dans la partie \@ref(subsec:word2vec), les couples `[focus, contexte]` dépendent d'un hyperparamètre : la fenêtre $w$.
...
1. Pour chaque phrase on effectue un sous-échantillonnage. Pour chaque mot $w_i$ on note $z(w_i)$ la proportion d'apparition de ce mot, c'est-à-dire le rapport entre le nombre de fois que ce mot apparaît et le nombre total de mots. La probabilité de garder un mot le mot $w_i$ est : 
$$
...
2. Pour chaque phrase on tire au hasard (selon une loi uniforme) un mot focus et on tire pour ce mot un mot *contexte* au hasard dans la fenêtre $w$ en imposant que les deux mots choisis soient parmi les mots sous-échantillonnés.

Par exemple, supposons que dans la phrase `[que, pensez, vous, de, ce, projet, i, e, qu, avez, vous, retenu, en, 10min]`, les mots sous-échantillonnés soient les mots en position 2, 5, 6, 8, 9, 10, 11, 12, 13, 14. 
Pour mieux comprendre, nous remplaçons les mots non échantillonnés par « nonsubsampled », mot qui a priori n'est pas dans le vocabulaire. La phrase devient `[nonsubsampled, pensez, nonsubsampled, nonsubsampled, ce, projet, nonsubsampled, e, qu, avez, vous, retenu, en, 10min]`.
Si $w=2$ alors le mot focus tiré ne peux être « pensez » puisque dans ce cas il n'y aurait aucun mot contexte associé. Si le mot focus tiré est « qu »  alors le mot contexte est tiré au hasard parmi `[e, avez, vous]`.
...
^[Étant donné le vecteur $z=(z_1,\,\dots,\,z_n)$) la fonction softmax est la fonction qui $z$ associé le vecteur $\sigma(z)$ dont la $j$^ème^ coordonnée est égale à $\sigma(z)_j=\frac{\exp(z_j)}{\sum_{i=1}^n\exp(z_i)}$.] :
$$
...
De manière équivalente à l'équation \@ref(eq:objSoftMax), qu'il faut maximiser, pour chaque couple `[focus, contexte]` on associe la fonction de perte suivante, qu'il faut minimiser :
$$
...
- Pour chaque couple `[focus, contexte]`, le calcul de $\log\mathbb P(w_{focus}\vert w_{context})$ est proportionnel à la taille du vocabulaire ;
...
C'est pourquoi la version *softmax* est très peu utilisée dans les implémentations de Skip-Gram. Une approche alternative, *negative sampling* avec une fonction sigmoïde, moins gourmande en temps de calcul, est alors souvent préféré^[
Une autre alternative à l'approche *softmax* parfois utilisée est l'approches *hierarchical softmax* qui se base sur l'utilisation d'arbres binaires de classification. La complexité de cet algorithme est proportionnelle à $\log_2n$ mais reste plus importante que celle de l'approche *negative sampling*.
].
...
Le *negative sampling* est basée sur le concept de *Noise Contrastive Estimation* --- estimation contrastée du bruit --- selon lequel un bon modèle logistique devrait être capable de différentier un faux signal d'un vrai signal.

Plutôt que mettre à jour l'ensemble des représentations vectorielles des mots pour chaque couple `[focus, contexte]`, dans l'approche *negative sampling* on tire $K$ mots au hasard du vocabulaire $(w_{neg,\,i})_{i=1..K}$, selon une distribution $P_n(w)$, en considérant que ces mots ne seront pas des mots contexte de `focus`^[
...
L'idée du *negative sampling* est donc de transformer un problème de classification multi-classes en un problème de classification binaire : pour chaque couple `[focus, contexte]`, on cherche à déterminer si le mot contexte est dans le contexte du mot focus ($D=1$, mot positif) ou non ($D=0$, mot négatif). 
...
\frac{1}{1+\exp(W_{e,w_{focus}}{}^tW_{s,w_{contexte}})}
$$
Par rapport à l'approche *softmax*, on cherche toujours à maximiser la quantité de l'équation \@ref(eq:objSoftMax) mais en estimant $\log\mathbb P(w_{focus}\vert w_{context})$ par :
$$
\mathbb P(w_{focus}\vert w_{context}) =
...
Pour chaque couple `[focus, contexte]` et un ensemble $(w_{neg,\,i})_{i=1..K}$ de mots négatifs tirés,  on associe la fonction de perte suivante :
$$
...
<!-- Ci-dessous quelques infos de Kim qui pourront peut-être t'inspirer tiré de : https://web.stanford.edu/~jurafsky/slp3/edbook_oct162019.pdf (regarder vers la page 115 sur word2vec) -->

<!--  word2vec est un modèle beaucoup plus simple que le modèle de langage de réseau de neurones, de deux manières. Tout d'abord, word2vec simplifie la tâche (ce qui en fait une classification binaire au lieu de la pré-diction des mots). Deuxièmement, word2vec simplifie l'architecture (formation d'un classificateur de régression logistique au lieu d'un réseau neuronal multicouche avec des couches cachées qui nécessitent des algorithmes de formation plus sophistiqués).  -->
...
## Description de la méthode et tests [Roro]

## Comparaison avec indicateurs insee
