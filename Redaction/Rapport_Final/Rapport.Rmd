...
Pour construire ce corpus, nous avons associé dix couples (du type `[voiture, camion]`), à dix mots différents que l'on considèrera comme des mots contextes (`[véhicule, moto\dots]`). Le corpus fictif est ainsi formé de 10 000 phrases composées chacune d'un mot d'un couple, de cinq mots du contexte et de trois mots bruits, tous tirés aléatoirement. L'objectif sera de vérifier qu'en appliquant notre modèle sur ce corpus fictif, on obtient bien des vecteurs-mots proches pour des couples `[focus, contexte]` et éloignés pour des couples `[focus, mots bruits]`.

Pour cela, nous avons mis en œuvre les différentes techniques d'évaluation^[À l'exception de la méthode par \og jugement humain \fg{} puisque le corpus est ici créé fictivement par ordinateur sans prêter attention au réel sens des mots.] 
...
L'idéal serait de disposer d'**une base de tweets traitant de sujets divers, et bien annotés**.  Il faudrait mobiliser un analyseur de sentiments performant pour les tweets en français, or peu de modèles de NLP préentraînés sont disponibles en dehors de l'anglais. Quelques avancées récentes, comme CamemBERT^[source], le cousin Français de BERT^[L'outil BERT de Google permet de comprendre les intentions formulées derrière les expressions naturelles des internautes en analysant les phrases et leur contexte.] entraîné sur 138GB de texte français issu du web, ...
...
Par exemple, « lol », « LOL » et « loool » sont peut-être utilisées dans des contextes différents : ironie, blagues, rires\dots L'extension *fasttext*^[
...
] (\cite{Bojanowski}) de *word2vec* apporte une solution au traitement des mots rares. On représente un mot non seulement comme lui-même mais aussi grâce à des sacs de $n$-grammes constitutifs. 
Si $n = 3$, « WHERE » est représenté non seulement par `<where>` mais également `<wh, whe, her, ere, re>`.
