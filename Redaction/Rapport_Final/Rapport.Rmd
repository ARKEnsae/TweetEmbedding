...
\end{center}
\captionsetup{margin=0cm,format=hang,justification=justified}
\caption{Architecture des modèles \emph{Continuous bags of words} (CBOW) et \emph{Skip-gram}.}\label{fig:cbowskipgram}
%\vspace{-0.3cm}
\footnotesize
...
Pour cette première série de tests d'hyperparamètres, nous avons fixé la dimension des *word-embeddings* à 50 et évalué l'impact du nombre d'epochs, de la taille de la fenêtre et du taux d'apprentissage (figure \ref{fig:evaluation_1}) . 
...
Nous avons ensuite lancé le modèle que nous avons implémenté en utilisant les paramètres retenus précédemment (w = 4, lr = 0,02 et dim = 100) mais uniquement sur 100 000 tweets et 80 epochs pour des questions de temps de calcul^[Pour ces paramètres, le temps de calcul était d'environ 18 heures.].
