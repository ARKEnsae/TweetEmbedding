...
Pour modéliser mathématiquement de manière relativement « grossière » un *domain shift*, il s’agirait de considérer par exemple une variable latente « idéale » $x_0$ (une base de tweets de référence), jamais observée mais qui influerait sur $y$ (l’indice mensuel). 
...
*A contrario*, l'indicateur synthétique issu de l'enquête Camme peut-être considéré comme CVS-CJO puisqu'il est construit à partir de soldes d'opinion CVS-CJO. Pour comparer notre indicateur à l'indicateur issu de Camme, deux solutions sont possibles :
  ...
  Pour connaître ces poinds, nous avons reproduit l'analyse factorielle utilisée pour construire l'indicateur synthétique publié par l'Insee. Faire une analyse factorielle statique revient à faire une moyenne pondérée des 8 soldes d'opinion centrés-réduits.
  ]. 
  ...
  Toutefois, de nombreuses méthodes économétriques ne sont applicables que sur les séries stationnaires \colorbox{BurntOrange}{Pour Alain : le terme stationnaire est-il juste ?} et donc désaisonnalisées, c'est pourquoi nous utiliserons dans ces cas la première approche. 
...
Dans cette partie, afin que les résultats économétriques soient valides, nous étudions les séries stationnaires, c'est-à-dire désaisonnalisées et différenciées. 
En effet, la figure \@ref(fig:bslogcam) suggère la présence d'une tendance stochastique dans tous les indicateurs (confirmée par les tests de Philipps-Perron et KPSS), y compris lorsqu'ils sont désaisonnalisés. \colorbox{BurntOrange}{Alain : regarder commentaire.}
<!-- Différencier les séries désaisonnalisées permet d'obtenir des séries stationnaires. => déjà dit plus haut, tu penses qu'il faut insister ?-->
...
En somme, l'exploitation des résultats de *word2vec* dans la construction d'un indice de sentiment de tweets permet, contrairement au modèle de référence, de construire un indicateur anticipé d'un mois permettant de prévoir (au sens de Granger) l'indicateur synthétique de l'enquête Camme. Cela est vrai alors même que les deux indicateurs évoluent différemment en raison de leur philosophie différente : alors que l'indicateur Camme résume l'opinion des ménages sur des sujets très spécifiques (évolution du niveau de vie, du chômage, etc.), notre indicateur de sentiment mesure juste la positivité / négativité des mots employés dans des tweets, sans cibler de thématique particulière.
...
Dans la partie \@ref(sec:sentimentalAnalysis) dédiée à l’analyse de sentiments, nous avons pu appliquer sur un cas concret plusieurs méthodes étudiées durant notre deuxième année à l'ENSAE (modèles de prédiction binaire, analyse de séries temporelles, minimisation du risque empirique / cross-validation\dots).
...
Au-delà de la positivité ou négativité d’un tweet, il demeure important d’également déceler les sujets sur lesquels les tweets portent pour créer un indice plus ciblé (via, par exemple, l'utilisation de *hashtags*).
...
L'objectif de la désaisonnaliser est de décomposer une série $X_t$ en trois composantes inobservées : la tendance-cycle, 
...
Parler en quelques mots de la méthode de désaisonnalisation et de la décomposition.

Parler du test combiné X-13, mettre des graphiques avec les coefficients.
