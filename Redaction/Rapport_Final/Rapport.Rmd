...
$$S_{1,\gamma}(t) = 2 \times \mathds{1}\left\{ \frac{1}{n} \sum \limits_{i=1}^n \alpha_i \geq \gamma\right\} - 1 \qquad \in [0,1]$$
 ...
 \colorbox{BurntOrange}{dire que si un mot apparait X fois dans une phrase positive il est compté X fois -> si quelqu'un a une idée pour le dire de façon claire Ok, sinon laisser tomber}  
...
*A contrario*, l'indicateur synthétique issu de l'enquête Camme peut-être considéré comme CVS-CJO puisqu'il est construit à partir de soldes d'opinion CVS-CJO. Pour comparer notre indicateur à l'indicateur issu de Camme, deux solutions sont possibles :
  ...
  Pour cela nous avons utilisé la méthode X-12ARIMA (voir annexe \@ref(annexe:cvscjo)).
...
Le graphique \@ref(fig:bslogcam) présente les indicateurs de sentiments construits à partir de notre modèle *word2vec*, celui construit à partir du modèle de référence et l'indicateur synthétique brut issu de l'enquête Camme (retardé).
Même si en niveau nos deux indicateurs de sentiments diffèrent de l'indicateur issu de l'enquête Camme, on observe plusieurs évolutions similaires (par exemple le pic dans tous les indicateurs entre mars et octobre 2012). 
...
\begin{figure}[htp]
\begin{center}
\colorbox{BurntOrange}{Rajouter graphique avec baseline SNCF, logit, Camme bruts} 
%\includegraphics[width=.6\textwidth]{img/cbow_skip_gram.png}
\captionsetup{margin=0cm,format=hang,justification=justified}
\caption{Indicateur synthétique brut de confiance des ménages de l'enquête Camme (retardé) et indicateurs mensuels de sentiments construits à partir de notre modèle \emph{word2vec} et à partir de notre modèle de référence.}\label{fig:bslogcam}
\end{center}
\footnotesize
...
Afin de regarder les similitudes entre nos indicateurs de sentiments et l'indicateur Camme, nous utilisons un algorithme de déformation temporelle dynamique --- *Dynamic Time Warping* (DTW) (voir par exemple \cite{dtw} pour l'implémentation en \faRProject).
...
<!-- Finalement je ne parle pas de ça puisqu'on en parle en conclu -->
...
- L'indicateur de sentiments issu du modèle *word2vec* cause au sens de Granger l'indicateur synthétique issus de l'enquête Camme (p-valeur de 0,02).
...
- L'indicateur de sentiments issu du modèle de référence ne cause pas au sens de Granger l'indicateur synthétique issus de l'enquête Camme (p-valeur de 0,58).
...
En somme, l'utilisation de la représentation vectorielle des mots permet bien
...
D’une part pour des raisons évoquées liées aux limites bases d’entraînement et de test (*domain-shift*, processus d’annotation, mots inconnus) mais également car au-delà de la positivité ou négativité d’un tweet, il demeure important d’également déceler le sujet sur lesquel le tweet porte pour créer un indice qui corresponde à une opinion sur un sujet précis ().
...
\colorbox{BurntOrange}{Paragraphe ouverture données manquantes + vocabulaire}
...
Afin de corriger des effets saisonniers et des effets jours ouvrables, la méthode X-12ARIMA fonctionne en deux étapes :

1. La série initiale est pré-ajustées de certains effets déterministes (effets jours ouvrables et points atypiques) grâce à un modèle de régression linéaire dont les résidus suivent un modèle ARIMA

2. Cette série pré-ajustée est ensuite désaisonnalisée par une méthode non-paramétrique qui repose sur l'usage de moyennes mobiles.


\colorbox{BurntOrange}{Rajouter graphiques}

## Correction des effets jours ouvrables
...
Pour nos indicateurs de sentiments (modèle logistique ou modèle de référence construit à partir du sentiment moyen des mots), nous ne détectons pas d'effet jours ouvrables. 
Cela ne signifie pas pour autant que le type de jour n'a pas d'effet sur nos indicateurs de sentiments, mais que nous n'arrivons pas à en estimer un.
...
On considère ainsi que l'effet d'un lundi lundi est le même que l'effet d'un mardi, etc. jusqu'au vendredi et que cet effet est différent le samedi et le dimanche. Le modèle \@ref(eq:eqcjo) devient :
\begin{eqnarray}
y_t=\beta_0 LY_t + \tilde\beta_1 \left(\sum_{j=1}^{5}N_{jt} - (N_{6t} + N_{7t})\right) + \varepsilon'_t
(\#eq:eqcjowd)
\end{eqnarray}

Dans notre cas, puisqu'il n'y a pas d'effet longueur du mois nous omettant le régresseur $LY_t$ dans \@ref(eq:eqcjo) et \@ref(eq:eqcwd). On considère qu'il y a un effet jours ouvrables significatif lorsque $\exists i\in \{1,2,\dots,6\}\::\:\beta_i\ne0$ ou lorsque $\tilde\beta_1\ne0$.


## Correction des effets saisonniers
