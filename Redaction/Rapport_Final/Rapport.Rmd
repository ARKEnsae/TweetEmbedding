...
La construction des axes de l'ACP est faite par projection orthogonale. Nous utilisons ici le produit scalaire $\langle x,y\rangle_{N} = ^t\!xN y$ avec la métrique $N = diag(\frac{1}{n},...,\frac{1}{n})$^[
La matrice $N$ représente une matrice de poids données à chaque individu. En général on donne un poids uniforme à chaque individu, d'où l'expression $N = diag(\frac{1}{n},...,\frac{1}{n})$.
]. 
Ainsi, la projection orthogonale d'un individu $i$ (vecteur ligne) $z_i$ sur une droite de vecteur directeur unitaire $v$ vaut $z_iv\times v$ et les coordonnées de projection des $n$ individus valent $Zv$.
