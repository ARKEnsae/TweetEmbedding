...
Le graphique \ref{fig:bslogcam} présente les indicateurs de sentiments construits à partir de notre modèle \emph{word2vec}, celui construit à partir du modèle de référence et l'indicateur synthétique brut issu de l'enquête Camme (retardé).
...
Afin de regarder les similitudes entre nos indicateurs de sentiments et l'indicateur Camme, nous utilisons un algorithme de déformation temporelle dynamique --- \emph{Dynamic Time Warping} (DTW) (voir par exemple \cite{dtw} pour l'implémentation en \faRProject).
...
\begin{itemize}
\item
  L'indicateur de sentiments issu du modèle \emph{word2vec} cause au sens de Granger l'indicateur synthétique issus de l'enquête Camme (p-valeur de 0,02).
...
D'une part pour des raisons évoquées liées aux limites bases d'entraînement et de test (\emph{domain-shift}, processus d'annotation, mots inconnus) mais également car au-delà de la positivité ou négativité d'un tweet, il demeure important d'également déceler le sujet sur lesquel le tweet porte pour créer un indice qui corresponde à une opinion sur un sujet précis ().
...
\def\labelenumi{\arabic{enumi}.}
\item
  La série initiale est pré-ajustées de certains effets déterministes (effets jours ouvrables et points atypiques) grâce à un modèle de régression linéaire dont les résidus suivent un modèle ARIMA
\item
  Cette série pré-ajustée est ensuite désaisonnalisée par une méthode non-paramétrique qui repose sur l'usage de moyennes mobiles.
\end{enumerate}

\colorbox{BurntOrange}{Rajouter graphiques}

\hypertarget{correction-des-effets-jours-ouvrables}{%
\subsection{Correction des effets jours ouvrables}\label{correction-des-effets-jours-ouvrables}}
...
\hypertarget{correction-des-effets-saisonniers}{%
\subsection{Correction des effets saisonniers}\label{correction-des-effets-saisonniers}}
