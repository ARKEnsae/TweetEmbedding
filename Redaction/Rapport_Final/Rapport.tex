...
Dans la suite de ce projet nous noterons \(n\) la taille du vocabulaire (i.e. : le nombre de mots différents) et \(d\) la dimension retenue pour \colorbox{BurntOrange}{les \emph{word-embeddings}}.
Comme décrit dans la partie \ref{subsec:word2vec}, l'approche \emph{Skip-gram} peut être vue comme un réseau de neurones avec trois couches avec : \colorbox{BurntOrange}{deux ou trois ? si trois, corriger aussi plus haut}

\begin{itemize}
\item
  En entrée une matrice \(W_e\) de taille \(n\times d\) ;
\item
  En sortie une matrice \(W_S\) de taille \(n\times d\).
\end{itemize}
...
est ensuite \emph{tokénisée} par la chaîne de caractère correspondant à un espace \texttt{"\ "} : on considère qu'il y a autant de mots de que chaînes de caractères séparées par un espace\footnote{Les mots composés sont donc considérés comme plusieurs mots distincts.}.

\begin{quote}
\LARGE \textbf{``}\normalsize \emph{Que pensez-vous de CE projet ?(i.e. : qu'avez-vous retenu en 10min?)} \LARGE \textbf{''}\normalsize
\end{quote}

Cette phrase est décomposée en 14 mots \texttt{{[}que,\ pensez,\ vous,\ de,\ ce,\ projet,\ i,\ e,\ qu,\ avez,\ vous,\ retenu,\ en,\ 10min{]}}.
...
\tightlist
\item
  Pour chaque phrase on effectue un sous-échantillonnage (\emph{sub-sampling}). Pour chaque mot \(w_i\) on note \(z(w_i)\) la proportion d'apparition de ce mot, c'est-à-dire le rapport entre le nombre de fois que ce mot apparaît et le nombre total de mots. La probabilité de garder un mot le mot \(w_i\) est :
  \[
  \mathbb P(w_i) = \min\left\{\left(\sqrt{\frac{z(w_i)}{q}} + 1 \right)
  \times
  \frac{q}{z(w_i)},1\right\}
  \]
  Le paramètre \(q\) appelé «~sample~» -- échantillonnage -- contrôle le nombre de sous-échantillonnages \colorbox{BurntOrange}{pas clair, expliciter, éventuellement avec une ndbp}. Si \(q\) vaut 0,001 (valeur par défaut) alors par exemples :
\end{enumerate}
...
\tightlist
\item
  Pour chaque phrase, on tire au hasard (selon une loi uniforme) un mot focus pour lequel on tire un mot \emph{contexte} au hasard dans la fenêtre \(w\) en imposant que les deux mots choisis soient parmi les mots sous-échantillonnés \colorbox{BurntOrange}{sinon on retire ? éventuellement ndbp}.
\end{enumerate}
...
\item
  \(w\) est la fenêtre ;
\item
  les \(w_{t+j}\) sont les mots voisins de \(w_t\) (\(w_t\) est donc un mot contexte et \(w_{t+j}\) un mot focus) ; \colorbox{BurntOrange}{être bien sûr que c'est pas l'nverse mais si ça se trouve c'est ça}.
\item
  ...
  \]
  avec \(W_{e,w_i}\) et \(W_{s,w_i}\) les représentations vectorielles du mot \(w_i\) respectivement dans la matrice d'entrée et de sortie et \(n\) la taille du vocabulaire.
\end{itemize}
...
\begin{itemize}
\item
  Pour chaque couple \texttt{{[}focus,\ contexte{]}}, la complexité du calcul de \(\log\mathbb P(w_{focus}\vert w_{context})\) est proportionnelle à la taille du vocabulaire ;
\item
  La complexité du calcul de \(\nabla_\theta \log\mathbb P(w_{focus}\vert w_{context})\) dans l'étape de descente de gradient est aussi proportionnelle à la taille du vocabulaire. \colorbox{BurntOrange}{à conserver ? Alain}
\end{itemize}
...
Le \emph{negative sampling} est basé sur le concept de \emph{Noise Contrastive Estimation} -- estimation contrastée du bruit -- selon lequel un bon modèle logistique devrait être capable de différentier un faux signal d'un vrai signal \colorbox{BurntOrange}{à clarifier ?}.

Dans cette approche, plutôt que de mettre à jour l'ensemble des représentations vectorielles des mots pour chaque couple \texttt{{[}focus,\ contexte{]}}, on tire \(K\) mots au hasard du vocabulaire \((w_{neg,\,i})_{i=1..K}\), selon une distribution \(P_n(w)\), en considérant que ces mots ne seront pas des mots voisins de \texttt{focus}\footnote{Il est bien sûr possible que parmi les mots tirés au hasard il y ait des mots qui soient vraiment dans le contexte. Cependant, puisque la taille du vocabulaire est très grande, on considère que cette erreur est négligeable.}.
L'idée du \emph{negative sampling} est donc de transformer un problème de classification multi-classes en un problème de classification binaire : pour chaque couple \texttt{{[}focus,\ mot2{]}}, on cherche à déterminer si \texttt{mot2} est dans le contexte de \texttt{focus}. Si c'est le cas, alors la décision \(D=1\) (mot2 est positif et est le \texttt{contexte}), sinon \(D=0\) (mot négatif, appartient à \((w_{neg,\,i})_{i=1..K}\)).
