...
\subsection{\texorpdfstring{Le modèle \emph{Word2vec}, un modèle de \emph{word-embedding} {[}Kim{]}}{Le modèle Word2vec, un modèle de word-embedding {[}Kim{]}}}\label{le-moduxe8le-word2vec-un-moduxe8le-de-word-embedding-kim}}

\hypertarget{lalgorithme-skip-gram-alain}{%
\subsection{L'algorithme Skip-Gram {[}Alain{]}}\label{lalgorithme-skip-gram-alain}}

Le modèle \emph{Word2Vec} est une des méthodes de machine learning utilisée pour faire du \emph{word embedding} --- plongement lexical.
Elle permet de représenter, de manière vectorielle, l'ensemble des mots d'un corpus.
Ainsi, les mots qui apparaissent dans des contextes similaires (par exemple «~bonjour~» et «~tsalut~») seront représentés par des vecteurs relativement proches.
L'objectif de cette partie est de décrire le fonctionnement de cet algorithme.

Deux architectures du modèles \emph{Word2Vec} existent (voir graphique \ref{fig:cbowskipgram}) :
...
\item
  L'approche \emph{Skip-gram} a un objectif inverse : estimer, pour chaque mot du vocabulaire, la probabilité d'être proche du mot \emph{focus}. C'est cette approche que nous étudions dans ce projet.
\end{itemize}
...
\hypertarget{analyse-des-sentiments}{%
\section{Analyse des sentiments}\label{analyse-des-sentiments}}

\hypertarget{description-de-la-muxe9thode-et-tests-partie-ruxe9diguxe9e-par-roro}{%
\subsection{Description de la méthode et tests {[}Partie rédigée par Roro{]}}\label{description-de-la-muxe9thode-et-tests-partie-ruxe9diguxe9e-par-roro}}
