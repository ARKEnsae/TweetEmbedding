...
\item
  ...
  \frac{
  \exp(W_s[focus,]\times {}^tW_e[context,])
  }{
  \sum_{i=1}^n\exp(W_s[i,]\times {}^tW_e[context,])
  }
  \]
  avec \(W_e[i,]\) et \(W_s[i,]\) sont les représentations vectorielles du mot \(i\) dans la matrice d'entrée et de sortie et \(n\) la taille du vocabulaire.
\end{itemize}
