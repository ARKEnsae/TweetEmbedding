...
tweet, à partir des mots qui le composent. Nous nous intéresserons ici à
2 sentiments possibles : +1 quand le tweet est jugé positif, -1 quand il
est jugé négatif. À partir de la base de tweets de la SNCF, nous allons
donc chercher à construire le meilleur modèle de prédiction de
...
Afin d'étudier l'ajustement du modèle, nous séparons le corpus en deux :
une base de \emph{train} composée de 16 004 tweets et une base de
...
d'utiliser un modèle de régression binaire estimé sur notre corpus, en
considérant comme prédicteurs chacune des dimensions des vecteurs (nous
en aurons donc ici \(n=100\)). Pour réaliser cette régression, il faut
toutefois pouvoir attribuer à chaque tweet un vecteur et effectuer une
forme de \emph{sentence embedding}. Pour cela, nous décidons d'attribuer
à chaque tweet la moyenne des vecteurs de l'ensemble des mots du tweet.
...
embedding} associé au tweet, alors on s'intéresse au modèle
\(Y_i = 1\{ \sum\limits_{i = 1}^n \beta_i X_{i,j} + \epsilon_i \geq 0 \}\)
où \(\epsilon_i\) est le résidu de notre modèle. En notant
\(F_{\epsilon}\) la fonction de répartition des résidus, nous allons
chercher à estimer :
...
interroger sur d'autres critères à propos du modèle :

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\item
  Les \emph{stop words}\footnote{La liste des \emph{stop wods} utilisés
    ici est la suivante : `ai', `aie', `aient', `aies', `ait', `as',
    `au', `aura', `aurai', `auraient', `aurais', `aurait', `auras',
    `aurez', `auriez', `aurions', `aurons', `auront', `aux', `avaient',
    `avais', `avait', `avec', `avez', `aviez', `avions', `avons',
    `ayant', `ayante', `ayantes', `ayants', `ayez', `ayons', `c', `ce',
    `ces', `d', `dans', `de', `des', `du', `elle', `en', `es', `est',
    `et', `eu', `eue', `eues', `eurent', `eus', `eusse', `eussent',
    `eusses', `eussiez', `eussions', `eut', `eux', `eûmes', `eût',
    `eûtes', `furent', `fus', `fusse', `fussent', `fusses', `fussiez',
    `fussions', `fut', `fûmes', `fût', `fûtes', `il', `ils', `j', `je',
    `l', `la', `le', `les', `leur', `lui', `m', `ma', `mais', `me',
    `mes', `moi', `mon', `même', `n', `ne', `nos', `notre', `nous',
    `on', `ont', `ou', `par', `pas', `pour', `qu', `que', `qui', `s',
    `sa', `se', `sera', `serai', `seraient', `serais', `serait',
    `seras', `serez', `seriez', `serions', `serons', `seront', `ses',
    `soient', `sois', `soit', `sommes', `son', `sont', `soyez',
    `soyons', `suis', `sur', `t', `ta', `te', `tes', `toi', `ton', `tu',
    `un', `une', `vos', `votre', `vous', `y', `à', `étaient', `étais',
    `était', `étant', `étante', `étantes', `étants', `étiez', `étions',
    `été', `étée', `étées', `étés', `êtes'} : également appelé \og mots
  vides \fg, ce sont des mots communs dont l'analyse est en général
  inutile car leur présence n'apporte pas d'information. Dans notre cas,
  on peut donc penser qu'estimer le modèle en enlevant du corpus ces
  \emph{stop-words} peut améliorer les résultats.
\item
  ...
  un vecteur commun à tous ces mots. On peut donc décider d'attribuer ce
  vecteur aux mots inconnus, en estimant que s'ils sont absents du
  corpus d'entraînement du modèle \emph{Word2Vec}, ce sont bien des mots
  très peu fréquents.
\end{enumerate}
...
\item
  Le vecteur nul ou le vecteur des mots très peu fréquents estimé par
  \emph{Word2Vec} pour les mots inconnus ;
\item
...
\item
  ...
  renvoient à des sentiments différents.
\item
...
\subsubsection{Comparaison entre les deux
modèles}\label{comparaison-entre-les-deux-moduxe8les}

L'application du modèle basé sur les \emph{word-embeddings} à la même
base \emph{test} que le modèle basé sur les sentiments moyens des mots
donne une \emph{accuracy} de \(68,71\%\), contre les \(89,08\%\) que
l'on avait obtenu. Nous obtenons donc de moins bons résultats en
utilisant le modèle basé sur les \emph{word-embeddings}, nous allons
essayer de comprendre pourquoi.

\paragraph{Influence des mots
inconnus}\label{influence-des-mots-inconnus}

Intuitivement, on peut penser que la prédiction du sentiment est
biaisée, dans les deux modèles, par la présence de mots inconnus. En
effet, pour le modèle basé sur les sentiments moyens, il y a un problème
de mots inconnus entre la bae \emph{test} et la base \emph{train} et
pour le modèle basé sur les \emph{Word2Vec}, il y a un problème de mots
inconnus entre la base SNCF et la base d'entraînement du
\emph{Word2Vec}. Pour vérifier si l'intuition se vérifie, nous regardons
la distribution des mots connus et inconnus entre les tweets bien
prédits, mal prédits et en général (figure \ref{fig:mots_inconnus}).
...
On remarque que la distribution des mots inconnus semble comparable dans
les tweets bien prédits et mal prédits, cela ne semble donc pas être
l'explication principale des résultats.

\paragraph{\texorpdfstring{Une cause des mauvaises prédictions : le
\emph{domain
