...
Grâce à l'évolution des méthodes d'apprentissage profond (\emph{Deep Learning}), l'appréhension du langage naturel est aujourd'hui devenu une discipline à part entière (\emph{Natural Language Processing}).
...
Les méthodes de \emph{word embedding} (« plongement lexical » en français) permettent de représenter chaque mot d'un dictionnaire par un vecteur de nombres réels afin que les mots qui apparaissant dans des contextes similaires possèdent des vecteurs correspondants qui sont relativement proches.
...
Le « Natural Language Processing » (NLP ou « traitement automatique du langage naturel ») est une branche de \emph{machine learning} visant à analyser, traiter et reproduire le langage humain. Les modèles de NLP \emph{Word2Vec}, développés par une équipe de recherche chez Google (\cite{Mikolov}), sont parmi les plus célèbres et utilisent le \emph{word-embedding} --- plongement lexical en français.
...
Les premiers modèles sémantiques\footnote{Comme le \emph{term frequency-inverse document frequency} (TF-IDF)} représentaient les relations entre mots grâce à des très grandes matrices dont les dimensions correspondaient à la taille du vocabulaire (contenant donc beaucoup de 0 et dites « sparses »). Les méthodes de \emph{word-embedding} qui sont ensuite apparus ont permis de représenter chaque mot d'un dictionnaire par un vecteur de nombres réels denses (peu de 0) de plus faible dimension (en général entre 50 et 1000). Si la réduction de dimension rend les vecteurs-mots moins facilement interprétables, elle a pour grand avantage de faciliter et d'accélérer les tâches d'apprentissage impliquant ces mots.
...
\begin{itemize}
\item
  L'approche \emph{Continuous bags of words} dont l'objectif est d'estimer la probabilité d'observer un mot, appelé \emph{focus}, sachant que le contexte dans lequel il apparait (i.e. : les mots voisins).
\item
...
Les voisins d'un mot focus reposent sur un hyperparamètre : la fenêtre (window ou \(w\)). Pour \(w = n\), les voisins du mot focus sont les \(n\) mots précédents et les \(n\) mots suivants dans la phrase. Par exemple, dans la phrase :
...
Ces deux matrices, initialisées en générant des lois normale \(\mathcal N(0,1)\), sont mises à jour à partir de couples \texttt{{[}focus,\ contexte{]}} construits (voir partie \ref{subsec:baseentrainement}).
...
\hypertarget{descente-de-gradient}{%
\subsubsection{Descente de gradient}\label{descente-de-gradient}}
...
\hypertarget{version-negative-sampling-1}{%
\subsubsection{Version negative sampling}\label{version-negative-sampling-1}}
