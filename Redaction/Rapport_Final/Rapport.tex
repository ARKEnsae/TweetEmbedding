...
L'idée n'est pas ici de modéliser mathématiquement \colorbox{BurntOrange}{NDBP à conserver ?}\footnote{Pour modéliser mathématiquement de manière relativement «~grossière~» un \emph{domain shift}, il s'agirait de considérer par exemple une variable latente «~idéale~» \(x_0\) (une base de tweets de référence), jamais observée mais qui influerait sur \(y\) (l'indice mensuel).
...
Cela peut venir du fait que, dans les tweets utilisés pour construire les indicateurs mensuels, il y a bien plus de mots inconnus au modèle de référence basé sur les tweets SNCF (20,7 \%) qu'à notre modèle \emph{word2vec} qui comporte de nombreux \emph{vecteurs-mots} (13,6 \%).
...
Au-delà de la positivité ou négativité d'un tweet, il demeure important d'également déceler les sujets sur lesquels les tweets portent pour créer un indice plus ciblé (via, par exemple, l'utilisation de \emph{hashtags}).
...
Par exemple, son extension \emph{fasttext}\footnote{La bibliothèque open-source \url{https://fasttext.cc/} permet de télécharger des modèles pré-entraînés dans 157 langues différentes.} (\cite{Bojanowski}), traite de la rareté des mots en représentant le mot non seulement comme lui-même mais grâce à des sacs de \(n\)-grammes constitutifs.
Par exemple, si \(n = 3\), «~WHERE~» est représenté non seulement par \texttt{\textless{}where\textgreater{}} mais également \texttt{\textless{}wh,\ whe,\ her,\ ere,\ re\textgreater{}}. \emph{word2vec} connaît également des modèles «~concurrents~», le plus connu étant certainement \emph{GloVe} pour «~\emph{Global Vectors}~» (\cite{Pennington}).
